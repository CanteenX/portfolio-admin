...
export function ImportExcelButton({ columns, templateName, onImport }) {
  ...
  const dismiss = () => { setImporting(false); setResult(null); setProgress(0); setTotal(0); };
  ...
  return (
    <>
      <div className="flex gap-1">
        <Button variant="outline" size="sm" onClick={() => downloadTemplate(columns, templateName)} className="font-bold uppercase tracking-wider rounded-sm text-xs">
          <FileDown className="w-3.5 h-3.5 mr-1.5" /> Template
        </Button>
        <Button variant="outline" size="sm" onClick={() => fileRef.current?.click()} disabled={importing} className="font-bold uppercase tracking-wider rounded-sm text-xs">
      ...
      {importing && (
        <div className="fixed bottom-0 right-0 left-0 lg:left-64 z-40 border-t border-border bg-card/95 backdrop-blur-sm shadow-lg">
          ...
          <div className="flex items-center justify-between px-4 py-2.5">
            {!isDone ? (
              ...
              <>
                <div className="flex items-center gap-4 text-sm">
                  {result.success > 0 && (
                    <span className="flex items-center gap-1.5 text-green-500">
                      <CheckCircle2 className="w-4 h-4" /> {result.success} imported
                    </span>
                  )}
                  {result.failed > 0 && (
                ...
                <Button variant="ghost" size="sm" onClick={dismiss} className="h-7 px-2 text-xs">
